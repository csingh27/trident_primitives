...
def sample(RNG, dataset, num_ways, num_shots, num_tests=1):
    ...
    for label, concept_arg in enumerate(random_concepts):
        ...
        labels = np.full(num_samples, label)  # TODO class labels
...
if __name__ == "__main__":
    ...
    from usertypes import (LIGHT_VARIABLE, FLOOR_VARIABLE,
                           Sample, name_to_classarg)
    ...
    concept_arg, shot_arg = 7, 1
    image, depth, label = dataset[concept_arg][shot_arg]
    ...
    diffuse = 0.9
    ambient = 0.1
    metadata = parse_metadata('datasets', 'PRIMITIVES')
    camera_origin = jp.array(metadata['camera_origin'])
    viewport_size = metadata['image_shape']
    y_FOV = metadata['y_FOV']
    shadow = False

    images, depths, labels = flatten(dataset)
    labels = parse_labels(labels, name_to_classarg, diffuse, ambient)

    render = SCENE(camera_origin, viewport_size, y_FOV, shadow)
    floor = FLOOR_VARIABLE(jp.full(3, 1.0), 0.1, 0.9)
    light = [LIGHT_VARIABLE(0.35, jp.array([-0.45, 0.45, 0.0])),
             LIGHT_VARIABLE(0.35, jp.array([0.45, 0.45, 0.0]))]

    arg = 77
    sample = Sample(*labels[arg])
    pred_image, depth = render(sample, light, floor)
    pred_image = np.array(pred_image)
    true_image = np.array(images[arg])
    error_image = (true_image - pred_image)**2
    image = np.concatenate([error_image, true_image, pred_image], axis=1)
    plt.imshow(image)
    plt.show()
